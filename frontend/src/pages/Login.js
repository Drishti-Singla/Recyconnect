...
import { userAPI, authUtils, socketService } from '../services/api';
...
function Login() {
  ...
  // State to control suspended account warning modal
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Store the JWT token in sessionStorage (clears on browser close)
      sessionStorage.setItem('authToken', token);

      // Store user data in sessionStorage (clears on browser close)
      ...
      console.log('🔐 isAdminUser calculated as:', isAdminUser);
      ...
      // Navigate based on admin status with forced redirect
      if (isAdminUser) {
        ...
        const targetUrl = returnUrl || '/explore';
        console.log('✅ Regular user - Navigating to', targetUrl);
        setTimeout(() => {
  ...
  // Handle contact admin button click - opens email client with pre-filled message
  // Includes user's email in the body for admin to identify the account
  const handleContactAdmin = () => {
    window.location.href = 'mailto:admin@chitkara.edu.in?subject=Account%20Suspension%20Appeal&body=Hello%20Admin,%0A%0AI%20would%20like%20to%20appeal%20my%20account%20suspension.%0A%0AEmail:%20' + email;
  };
          ...
          <div style={{
            ...
            <div style={{
              ...
              <button
                ...
                onMouseLeave={(e) => e.target.style.backgroundColor = '#ffa502'}
              >
                📧 Contact Admin
              </button>
