...
function Dashboard() {
  ...
  // Load user data from sessionStorage (or localStorage as fallback)
  useEffect(() => {
    ...
    console.log('sessionStorage user:', loggedInUser);
    ...
    // Set user data from sessionStorage immediately
    setUser({
      ...userData,
      name: userData.username || userData.name, // Map username to name for display
      avatar: logo,
    ...
    const fetchUserFromDB = async () => {
      ...
      try {
        ...
        const dbUser = profileResponse.user || profileResponse; // Handle both {user: {...}} and direct user object
        console.log('User ID for loading items:', dbUser.id);

        // Update user data with fresh data from backend
        setUser({
          ...dbUser,
          name: dbUser.username || dbUser.name, // Map username to name for display
          avatar: logo,
          lastLogin: new Date().toLocaleString(),
  ...
  const loadUserItems = async (userId) => {
    ...
    try {
      ...
      if (duplicateIds.length > 0) {
        console.error('⚠️ DUPLICATE IDS FOUND:', duplicateIds);
      }

      // Map snake_case fields to camelCase for Dashboard compatibility
      ...
      console.log('User items set in state, count:', mappedItems.length);
    } catch (error) {
  ...
  const loadUserLostFoundItems = async () => {
    ...
    try {
      ...
      const items = response.reportedItems || [];
      ...
      setUserLostFoundItems(mappedItems);
  ...
  const loadUserDonatedItems = async () => {
    ...
    try {
      ...
      // Handle both response.donatedItems and response.data formats
      const donatedItems = response.donatedItems || response.data?.donatedItems || response.data || (Array.isArray(response) ? response : []);
      ...
      if (Array.isArray(donatedItems)) {
        // Map snake_case fields to camelCase for Dashboard compatibility
        const mappedItems = donatedItems.map(item => ({
          ...item,
        ...
        setUserDonatedItems(mappedItems);
  ...
  const handleSaveEditItem = async () => {
    ...
    try {
      const updatedItem = {
        ...
        askingPrice: editingItem.editedAskingPrice ? parseFloat(editingItem.editedAskingPrice) : 0,
  ...
  const handleUpdateProfile = async () => {
    ...
    try {
      // Use updateProfile instead of updateUser, and map 'name' to 'username'
      const updateData = {
        username: editFormData.name,
        phone: editFormData.phone,
        bio: editFormData.bio, // Save bio properly
      };
      ...
      setUser({
        ...refreshedUser,
        name: refreshedUser.username, // Map username back to name for display
        avatar: logo,
        lastLogin: new Date().toLocaleString(),
      ...
      setUpdateMessage('Failed to update profile: ' + (error.message || 'Please try again.'));
    } finally {
  ...
  const sendMessage = async () => {
    ...
    try {
      ...
      // Send via REST API (which will be the single source of truth)
      ...
      if (response.ok) {
        // Optionally notify via socket (if backend doesn't already do it)
        // The socket listener will handle updating the UI
        // Just reload to ensure we have the message
  ...
  // Load conversations when messages section is active
  useEffect(() => {
    if (activeSection === 'messages') {
      ...
      const handleNewMessage = (message) => {
        ...
        // If the message is from the current conversation, reload messages
        if (selectedConversation && 
            (message.sender_id === selectedConversation.other_user_id || 
             message.recipient_id === selectedConversation.other_user_id)) {
          loadChatMessages(selectedConversation.other_user_id);
        }
      ...
      // Cleanup: remove listener when leaving messages section
      return () => {
  ...
  useEffect(() => {
    if (selectedConversation) {
      ...
      // Polling is not needed since we have socket for real-time updates
      // Only poll as a fallback if socket is not connected
      let messagePolling;
      if (!socketService.isConnected()) {
        ...
        }, 5000); // Poll every 5 seconds only if socket is down
      }
  ...
  // Handle URL parameters for initial section and auto-open chat
  useEffect(() => {
    const params = new URLSearchParams(location.search);
    const section = params.get('section');
    const userId = params.get('userId');
    
    if (section && ['profile', 'messages', 'items', 'donations', 'reportedItems', 'concerns'].includes(section)) {
      setActiveSection(section);
    }
    
    // If userId is provided, automatically open chat with that user
    if (userId && section === 'messages') {
      // Create or find conversation with this user
      const startConversationWithUser = async () => {
        try {
          const token = sessionStorage.getItem('authToken') || localStorage.getItem('authToken');
          
          // First, load conversations to see if one exists
          ...
          if (existingConv) {
            ...
            // Fetch user info to create new conversation
            ...
            const newConv = {
              ...
              other_user_name: otherUser.username || otherUser.name || 'User',
  ...
  const renderMessagesSection = () => (
    <div>
      <h3 style={{ color: currentColors.text, margin: 0, marginBottom: '1.5rem', borderBottom: `2px solid ${currentColors.primary}`, paddingBottom: '0.5rem' }}>💬 My Messages</h3>
      
      <div style={{ display: 'grid', gridTemplateColumns: selectedConversation ? '300px 1fr' : '1fr', gap: '1rem', height: '600px' }}>
        {/* Conversations List */}
        <div style={{ background: currentColors.highlight, borderRadius: 8, padding: '1rem', overflowY: 'auto' }}>
          <h4 style={{ color: currentColors.text, marginBottom: '1rem', fontSize: '1rem' }}>Conversations</h4>
          ...
          {isLoadingConversations ? (
            ...
            <div style={{ textAlign: 'center', padding: '2rem', color: currentColors.textSecondary }}>
              ...
              <p style={{ fontSize: '0.9rem' }}>Start chatting with sellers on items you're interested in!</p>
            </div>
          ) : (
            conversations.map(conv => (
              <div
                ...
                style={{
                  padding: '0.75rem',
                  marginBottom: '0.5rem',
                  background: selectedConversation?.other_user_id === conv.other_user_id ? currentColors.primary : currentColors.cardBackground,
                  color: selectedConversation?.other_user_id === conv.other_user_id ? '#fff' : currentColors.text,
                  borderRadius: 6,
                  cursor: 'pointer',
                  transition: 'all 0.2s',
                  position: 'relative'
              ...
              >
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '0.25rem' }}>
                  <div style={{ fontWeight: 600 }}>
                    {conv.other_username || conv.other_email || 'User'}
                  </div>
                  {conv.unread_count > 0 && (
                    <span style={{
                      background: '#ff4444',
        ...
        {/* Chat Messages */}
        {selectedConversation && (
          <div style={{ background: currentColors.cardBackground, borderRadius: 8, padding: '1rem', display: 'flex', flexDirection: 'column' }}>
            <div style={{ borderBottom: `1px solid ${currentColors.border}`, paddingBottom: '0.75rem', marginBottom: '1rem' }}>
              <h4 style={{ color: currentColors.text, margin: 0 }}>
                {selectedConversation.other_username || selectedConversation.other_email}
              </h4>
            </div>
                  ...
                  >
                    <div
                      style={{
                        ...
                        padding: '0.75rem',
                        borderRadius: 12,
                        background: msg.sender_id === user.id ? currentColors.primary : currentColors.highlight,
                        color: msg.sender_id === user.id ? '#fff' : currentColors.text
                      }}
                    >
                      <div>{msg.content}</div>
                ...
                <div style={{
                  ...
                  <div style={{ display: 'flex', gap: '0.5rem', flexWrap: 'wrap' }}>
                    {item.status !== 'sold' && (
                      ...
                      onClick={async () => {
                        try {
                          await itemAPI.updateItem(item.id, { status: 'sold' });
                          await loadUserItems(user.id);
            ...
            <div key={item.id} style={{ 
              ...
              {editingDonatedItem && editingDonatedItem.id === item.id ? (
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                  <div style={{ flex: 1 }}>
                    <h4 style={{ margin: 0, marginBottom: '0.5rem', color: currentColors.text }}>
                      {item.title}
                    </h4>
                  ...
                  <div style={{ display: 'flex', gap: '0.5rem', flexWrap: 'wrap' }}>
                    ...
                    {item.status !== 'archived' && (
                      <button
                        onClick={async () => {
                          try {
                            await donatedItemAPI.updateStatus(item.id, { status: 'archived' });
                            await loadUserDonatedItems();
  ...
  const renderLostFoundSection = () => (
    <div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(150px, 1fr))', gap: '1rem', marginBottom: '1.5rem' }}>
        ...
        <div style={{ background: currentColors.highlight, padding: '1rem', borderRadius: 8, textAlign: 'center' }}>
          <div style={{ fontSize: '1.5rem', fontWeight: 'bold', color: currentColors.danger }}>{userLostFoundItems.filter(item => item.report_type === 'lost').length}</div>
          <div style={{ fontSize: '0.9rem', color: currentColors.textSecondary }}>Lost Items</div>
        </div>
        <div style={{ background: currentColors.highlight, padding: '1rem', borderRadius: 8, textAlign: 'center' }}>
          <div style={{ fontSize: '1.5rem', fontWeight: 'bold', color: currentColors.success }}>{userLostFoundItems.filter(item => item.report_type === 'found').length}</div>
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                  <div style={{ flex: 1 }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', marginBottom: '0.5rem' }}>
                      <h4 style={{ color: currentColors.text, margin: 0 }}>{item.title}</h4>
                      <span style={{ 
                        background: item.report_type === 'lost' ? currentColors.danger : currentColors.success,
                        color: '#fff',
                        padding: '2px 8px',
                        borderRadius: 12,
                        fontSize: '0.8rem',
      ...
      {!isEditing ? (
        ...
        <div>
          ...
          <div style={{ marginBottom: '1.5rem' }}>
            <label style={{ display: 'block', marginBottom: 6, fontWeight: 500, color: currentColors.text }}>Username</label>
            <div style={{ ...inputStyle, background: currentColors.background, cursor: 'default' }}>{user?.username || 'Not set'}</div>
          </div>

          <div style={{ marginBottom: '1.5rem' }}>
            <label style={{ display: 'block', marginBottom: 6, fontWeight: 500, color: currentColors.text }}>Email</label>
            <div style={{ ...inputStyle, background: currentColors.background, cursor: 'default', display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
              {user?.email || 'Not set'}
          ...
          <div style={{ marginBottom: '1.5rem' }}>
            <label style={{ display: 'block', marginBottom: 6, fontWeight: 500, color: currentColors.text }}>Roll Number</label>
            <div style={{ ...inputStyle, background: currentColors.background, cursor: 'default' }}>{user?.roll_number || 'Not set'}</div>
          </div>
        ...
        <div>
          ...
          <div style={{ marginBottom: '1.5rem' }}>
            ...
            <input 
              ...
              disabled={true}
              style={{ ...inputStyle, backgroundColor: '#f5f5f5', cursor: 'not-allowed', color: '#666' }} 
              placeholder="Email cannot be changed"
              title="Email cannot be modified for security reasons"
            />
            <small style={{ color: '#888', fontSize: '12px' }}>Email cannot be changed for security reasons</small>
          </div>
          ...
          <div style={{ marginBottom: '1.5rem' }}>
            ...
            {editFormData.phone && editFormData.phone.length !== 10 && (
              <small style={{ color: '#ff4757', fontSize: '12px' }}>Phone must be 10 digits</small>
            )}
          </div>
    ...
    <div style={{ minHeight: '100vh', background: currentColors.background, padding: '2rem 1rem' }}>
      ...
      <div style={{ maxWidth: 1200, margin: '0 auto', marginBottom: '2rem' }}>
        ...
        <div style={{ background: currentColors.cardBackground, padding: '1rem', borderRadius: 12, boxShadow: `0 2px 8px ${currentColors.shadow}`, display: 'flex', alignItems: 'center', gap: 16 }}>
          <div>
            <h3 style={{ margin: 0, color: currentColors.text }}>{user?.username || user?.name}</h3>
      ...
      <div style={{ maxWidth: 1200, margin: '0 auto', display: 'grid', gridTemplateColumns: '250px 1fr', gap: '2rem' }}>
        ...
        <div style={{ background: currentColors.cardBackground, padding: '1.5rem', borderRadius: 12, boxShadow: `0 2px 8px ${currentColors.shadow}`, height: 'fit-content' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
            ...
            {sectionButton('messages', 'My Messages', '💬')}
