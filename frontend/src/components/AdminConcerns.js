...
function AdminConcerns() {
  ...
  const updateConcernStatus = async (concernId, newStatus, adminResponse = '') => {
    try {
      ...
      // Call API to update concern status
      await userConcernAPI.updateConcernStatus(concernId, updateData);
      
      // Reload concerns from server to ensure we have the latest data
      await loadConcerns();
      
      setShowModal(false);
      setResponseText('');
      console.log(`Successfully updated concern ${concernId} to ${newStatus}`);
      
      // Show success message (you can add a notification system if needed)
      alert(`Concern status successfully updated to ${newStatus}`);
    } catch (error) {
      console.error('Error updating concern:', error);
      alert(`Failed to update concern status: ${error.message}`);
    }
  };
